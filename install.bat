...
if errorlevel 1 (
    echo [提示] venv 创建失败，尝试 virtualenv 作为备选...
    %PYTHON_CMD% -m pip install --quiet virtualenv
    %PYTHON_CMD% -m virtualenv venv
    if errorlevel 1 (
        echo [错误] 虚拟环境创建失败
        echo 请从 https://www.python.org/downloads/ 重新安装官网版 Python
        echo 安装时务必勾选 "Add Python to PATH"
        pause
        exit /b 1
    )
...
echo  3. Windows 设置开启系统代理：127.0.0.1:8080
echo  4. 首次运行：浏览器访问 http://mitm.it 安装证书
echo  5. 打开微信 PC，刷新订阅号消息，文章自动入库
